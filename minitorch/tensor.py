...
class Tensor:
    ...
    def to_numpy(self) -> npt.NDArray[datatype]:
        """
        Return a NumPy view respecting this tensor's shape, strides, and offset.
        """
        storage = self._tensor._storage
        offset = getattr(self._tensor, "base_offset", 0)
        shape = self.shape
        strides = getattr(self._tensor, "_strides", None)
        if strides is None:
            # fallback to contiguous reshape
            return self.contiguous()._tensor._storage.reshape(shape)
        # byte strides = element_stride * itemsize
        byte_strides = tuple(int(s) * storage.itemsize for s in strides)
        flat = storage[offset:]
        return as_strided(flat, shape=shape, strides=byte_strides)
    ...
    def __getitem__(self, key: Union[int, UserIndex]):
        key2 = (key,) if isinstance(key, int) else key
        if any(isinstance(k, slice) for k in key2):
            td = self._tensor.slice_view(key2)
            return Tensor(td, backend=self.backend)
        return self._tensor.get(key2)

    def __setitem__(self, key: Union[int, UserIndex, Tuple[Union[int, slice], ...]], val) -> None:
        key2 = (key,) if isinstance(key, int) else key
        if any(isinstance(k, slice) for k in key2):
            assert isinstance(val, Tensor)
            self._tensor.store_view(key2, val._tensor)
        else:
    ...
    # ---- New functions ----
    def topk(self, k: int, dim: int) -> Tuple[Tensor, Tensor]:
        return topk(self, k, dim)
    
    def argsort(self, dim: int, descending: bool = False) -> Tensor:
